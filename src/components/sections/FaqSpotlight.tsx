					...
					{SPOTLIGHT_ITEMS.map((item, i) => {
						...
						return (
							<Reveal key={item.question} delay={i * 0.08}>
								<a
									href={`#faq-${item.slug}`}
									className="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-6 transition-all hover:-translate-y-0.5 hover:border-[#7c3aed]/40 hover:shadow-[0_8px_24px_rgba(124,58,237,0.08)]"
								>
									<p className="mb-3 flex items-center gap-1.5 font-mono text-[#7c3aed] text-xs uppercase tracking-[0.2em]">
									...
									<p className="mt-5 text-right font-semibold text-[#7c3aed] text-sm">
										전체 보기{" "}
