				...
				<div ref={gridRef} className="relative grid grid-cols-1 gap-12 md:grid-cols-4 md:gap-6">
					...
					{PROCESS_STEPS.map((step, i) => {
						...
						return (
							...
							>
								<div className="relative z-10 mb-6 flex h-20 w-20 items-center justify-center rounded-full border border-slate-200 bg-white shadow-[0_2px_8px_rgba(124,58,237,0.06)]">
									<span className="font-bold font-mono text-[#7c3aed] text-xl">
										{String(i + 1).padStart(2, "0")}
									</span>
								</div>

								<Icon className="mb-3 h-6 w-6 text-[#7c3aed]" aria-hidden="true" />
								...
								{step.badge && (
									<span className="mt-4 inline-block w-fit rounded-full bg-[#7c3aed]/10 px-3 py-1 font-semibold text-[#7c3aed] text-xs">
										{step.badge}
									</span>
								)}
							</motion.div>
