...
export const ProfessionalHero = () => {
	...
	return (
		<section className="relative flex min-h-screen items-center overflow-hidden bg-[#0a0e2e] px-6 py-24 md:px-10 md:py-32">
			<motion.div
				className="pointer-events-none absolute h-[500px] w-[500px] rounded-full bg-[#7c3aed] opacity-[0.18] blur-[130px]"
			...
			<div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 md:grid-cols-2">
				<div>
					<motion.p
						className="mb-4 font-semibold text-[#a78bfa] text-sm uppercase tracking-[0.25em]"
				...
				<div ref={chipsRef} className="grid grid-cols-3 gap-2 md:grid-cols-4">
					{CLIENT_DISCIPLINES.map((d, i) => (
						...
						>
							...
							<p className="mt-0.5 text-white/40 text-xs">{d.caption}</p>
						</motion.div>
					))}
				</div>
			</div>
